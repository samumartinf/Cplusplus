...
class DNA_DB{
  public:
    ...
    int cRegion = 0;
    int numBasePairs = 0;
    vector<pair<int, int>> nRegions;
    vector<pair<int, int>> cRegions;
...
DNA_DB::DNA_DB(string fileName){ //constructor for the class object
  ...
  for (size_t i = 0; i < geneCode.size(); i++) {
    ...
    if (i > 0 && geneCode[i] == 'N' && geneCode[i-1] != 'N') {
      nRegion++;
      nRegions.push_back(make_pair(i, 0));
      cRegions[cRegion-1].second = i-1;
    }
    if (i > 0 && geneCode[i] != 'N' && geneCode[i-1] == 'N') {
      cRegion++;
      cRegions.push_back(make_pair(i, 0));
      nRegions[nRegion-1].second = i-1;
    }
  }
